...
def main():
    ...
    st.set_page_config(
        page_title="Titanic: análisis de datos", layout="wide", page_icon="🚢")
    ...
    menu = [
        ...
        "Conociendo a los pasajeros",
        "Sobre el precio de los pasajes",
        "Ruta y ciudades de embarque",
        "In memoriam...",
    ]
    ...
    if choice == "Correlaciones entre los datos":
        ...
        hist_edad = px.histogram(
            ...
            nbins=40,
            template="plotly_dark",
            labels={"AGE": "Edad", "SURVIVED": "Superviviente"},
        ...
        kid.update_layout(
            title="Cantidad de niños por edad y si murieron o no",
            xaxis_title_text="Edad (años)",
            yaxis_title_text="Cantidad de niños",
            template="plotly_dark",
        )
        ...
        st.subheader("Sobre el precio de los pasajes")
        st.write(
        ...
        edad_precio = px.scatter(
            ...
            color_discrete_sequence=colores,
            template="plotly_dark",
            title="Relación entre la edad, el precio del pasaje y la supervivencia de los pasajeros",
            labels={
                "AGE": "Edad",
        ...
        passenger_class = (
            titan.groupby(["PCLASS", "SURVIVED"]).size().reset_index(name="count")
        )
        ...
        biglietti = px.bar(
            ...
            labels={
                ...
                "count": "Número de pasajeros",
                "SURVIVED": "Superviviente",
            },
            title="Número de pasajeros por tipo de pasaje",
        )
        ...
        tarifas = px.box(
            ...
            y="FARE",
            points="all",
            labels={"PCLASS": "Clase de pasajero", "FARE": "Tarifa del pasaje"},
            color="PCLASS",
            template="plotly_dark",
        ...
        embark = px.bar(
            ...
            labels={
                ...
                "PASSENGERID": "Cantidad de pasajeros",
            },
            template="plotly_dark",
            category_orders={"EMBARKED": order},
        )
        st.plotly_chart(embark, use_container_width=True)

        EMBARKED_survival_counts = (
            titan.groupby(["EMBARKED", "SURVIVED"])["PASSENGERID"].count().reset_index()
        )

        EMBARKED_survival_counts["Total"] = EMBARKED_survival_counts.groupby(
            ["EMBARKED"]
        )["PASSENGERID"].transform("sum")
        EMBARKED_survival_counts["Proportion"] = (
            EMBARKED_survival_counts["PASSENGERID"] / EMBARKED_survival_counts["Total"]
        )

        surv_embark = go.Figure()
        surv_embark.add_trace(
            go.Bar(
                x=EMBARKED_survival_counts[
                    EMBARKED_survival_counts["SURVIVED"] == "Dead"
                ]["EMBARKED"],
                y=EMBARKED_survival_counts[
                    EMBARKED_survival_counts["SURVIVED"] == "Dead"
                ]["Proportion"],
                name="No sobrevivió",
                text=EMBARKED_survival_counts[
                    EMBARKED_survival_counts["SURVIVED"] == "Dead"
                ]["PASSENGERID"],
        ...
        surv_embark.add_trace(
            go.Bar(
                x=EMBARKED_survival_counts[
                    EMBARKED_survival_counts["SURVIVED"] == "Alive"
                ]["EMBARKED"],
                y=EMBARKED_survival_counts[
                    EMBARKED_survival_counts["SURVIVED"] == "Alive"
                ]["Proportion"],
        ...
        st.image(
            "/Users/enriquebook/programas_python/Data_Analysis/13-Proyecto-Módulo-1/Titanic/img/wordcloud.png",
